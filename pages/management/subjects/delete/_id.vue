  ...
  export default {
    ...
    async fetch() {
      this.$data.schedules = (await this.$axios.$get(
        `https://api.ryzhenkov.space/v1/diary/schedules?subjectId=${this.$route.params.id}&limit=99999&page=1`
      )).results;
      this.$data.replacements = (await this.$axios.$get(
        `https://api.ryzhenkov.space/v1/diary/replacements?replacingSubject=${this.$route.params.id}&limit=99999&page=1`
      )).results;
    ...
    methods: {
      processDelete() {
        ...
        this.isLoading = true;
        this.$axios.$delete(`https://api.ryzhenkov.space/v1/diary/subjects/${this.$route.params.id}`).then((res) => {
