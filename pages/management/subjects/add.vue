<template>
  <div class="add">
    ...
    <div class="add--field">
      <p>
        Название предмета
        <span v-if="errors && errors.name" class="field--error">{{ errors.name }}</span>
      </p>
      <input
        id="teacher"
        v-model="name"
...
export default {
  ...
  methods: {
    sendData() {
      ...
      this.isLoading = true;
      this.$axios.$post(`${process.env.baseUrl}/v1/diary/subjects`, {
        name: this.name,
...
.add {
  ...
  .add--field {
    .add--input {
      @apply border-none rounded-2xl shadow-none w-full;
    }
