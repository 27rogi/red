...
export default {
  ...
  async fetch() {
    this.$data.subject = (await this.$axios.$get(
      `${process.env.baseUrl}/v1/diary/subjects/${this.$route.params.id}`
    ));
  ...
  methods: {
    sendData() {
      ...
      this.isLoading = true;
      this.$axios.$patch(`${process.env.baseUrl}/v1/diary/subjects/${this.$route.params.id}`, {
        name: this.name,
...
.editor {
  ...
  .editor--field {
    .editor--input {
      @apply border-none rounded-2xl shadow-none w-full p-2;
    }
