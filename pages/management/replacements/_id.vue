...
export default {
  ...
  async fetch() {
    this.$data.replacement = (await this.$axios.$get(
      `${process.env.baseUrl}/v1/diary/replacements/${this.$route.params.id}?extras=subject,schedule`
    ));
    ...
    this.$data.schedule.selectedSchedule = (await this.$axios.$get(
      `${process.env.baseUrl}/v1/diary/schedules/${this.replacement.replacedSchedule}?extras=subject,bell,replacements`
    ));
  },
  methods: {
    limitText(count) {
    ...
    findSchedules(query) {
      this.isLoading = true
      this.$axios.$get(
        `${process.env.baseUrl}/v1/diary/schedules?sortBy=scheduleId%3Aasc&limit=9999&extras=subject,bell,replacements`)
        .then(
          response => {
    ...
    sendData() {
      ...
      this.$axios.$patch(`${process.env.baseUrl}/v1/diary/replacements/${this.$route.params.id}`, patchBody).then((res) => {
