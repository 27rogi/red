<template>
  ...
  <div v-else class="homework--wrapper">
    <div class="homework--header">
      ...
      <nuxt-link
        class="button"
        :to="{ path: '/management/homeworks/' + homework.homeworkId, query: { 'diff': true } }"
      >Исправить задание</nuxt-link>
    </div>
    <div class="homework">
      <p class="homework--subject">Задание по предмету "{{ homework.subject.name }}"</p>
      <p class="homework--added">
        Добавлен:
        <span>{{ $moment(homework.created).format('DD MMMM YYYY') }}</span>
      </p>
      ...
      <div v-if="homework.files && homework.files.length > 0" class="homework--files">
        ...
        <div class="files">
          <a v-for="file in homework.files" :key="file" :href="file">
...
.homework--wrapper {
  ...
  .homework {
    @apply flex flex-col bg-primary-200 bg-opacity-50 rounded-2xl p-6 gap-4;
    .homework--subject {
    ...
    .homework--added {
      @apply font-semibold text-xl;
      span {
        @apply font-medium text-primary-900 text-opacity-60;
      }
    }
    .homework--files {
      .files--title {
