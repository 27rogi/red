            ...
            <template slot="table-row" slot-scope="props">
              <div v-if="props.column.field == 'actionButtons'" class="action-buttons">
                <NuxtLink
                  ...
                  :to="`/management/${item}/${props.row[`${itemId}`]}`"
                >Изменить</NuxtLink>
                <NuxtLink
                  class="button button__danger"
                  :to="`/management/${item}/delete/${props.row[`${itemId}`]}`"
                >Удалить</NuxtLink>
              </div>
            </template>
          </vue-good-table>
...
export default {
  ...
  async fetch() {
    ...
    this.$data.items = (await this.$axios.$get(
      `${process.env.baseUrl}/v1/diary/${this.$props.item}?sortBy=${this.$props.sort}%3Aasc&page=${currentPage}&limit=${currentLimit}${extras}`,
    ))
  },
  watch: {
    $route(to, from) {
...
.admin-panel {
  ...
  .action-buttons {
    @apply flex flex-col w-full gap-2 md:flex-row md:w-auto;
  }
}
</style>
